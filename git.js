    ...
    constructor(type, name, level, parent){
        ...
        if(isFile(type)){
            this.hash = generateHash(JSON.stringify({
                name, level, content: this.conent
            }))
        } else{
            this.hash = "";
        }
...
const secondDirectory = new Git(DIRECTORY, "second directory", root.level + 1 )

const file = new Git(FILE, "testFile", root.level + 1, root)
file.setContent("someData")

const file2 = new Git(FILE, "Hello world", root.level + 1, root)
const secondFile = new Git(FILE, "secondDirectoryFile", secondDirectory.level + 1, secondDirectory)
